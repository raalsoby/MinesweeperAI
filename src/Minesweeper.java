...
public class Minesweeper extends Applet implements Runnable {
	...
	public void simpleBot() {
		...
		while (sadness == bored) {
			...
			if (!moveMade) {
				...
				while (count < 1) {
					...
					if (exposed[t3 + t4 * width] == unexposed) {
						expose(t4, t3);
						count++;
						System.out.println("made a random move");
					}
				}
			}
			...
			System.out.println("There are " + (mines - flags) + " mines left");
			System.out.println("There are " + remaining + " squares remaining");
			// choose best one
			// play it
	...
	// if(exposed[2*i] != flagged) {exposed[2*i] = flagged; flags++;
	// paintFlags(getGraphics());}
